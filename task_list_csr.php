...
$qry = $conn->query("
    ...
    FROM task_list t 
    LEFT JOIN employee_list a ON t.assigned_by = a.id 
    LEFT JOIN users u ON t.assigned_by = u.id -- Assuming admin users are in a separate table named users
    LEFT JOIN pages p ON p.id = t.page_id 
    LEFT JOIN employee_list e ON FIND_IN_SET(e.id, t.employee_id) > 0
    LEFT JOIN department_list d ON e.department_id = d.id OR a.department_id = d.id
   WHERE d.id = 6
      AND (a.department_id IS NULL OR a.department_id != 8) 
      AND (e.department_id IS NULL OR e.department_id != 8)
...
// Check if query execution failed
    ...
    // Query executed successfully
        ...
        // print_r($row);
                     ...
                     <tr>
                        ...
                        <td>
                            <a href="edit_image.php?task_id=<?php echo $row['id']; ?>" target="_blank">
                                <b><?php echo isset($row['pagename']) ? ucwords($row['pagename']) : 'N/A'; ?></b>
                            </a>
                        </td>
    ...
    if ($row['status'] == 0) {
        ...
        echo "<span class='badge badge-info' id='onprogress-".$row['id']."' data-task='".$row['id']."'>On-Progress</span>";

    } elseif ($row['status'] == 1) {

        echo "<span class='badge badge-primary' id='pending-".$row['id']."' data-task='".$row['id']."'>Pending</span>";

    } elseif ($row['status'] == 2) {

        echo "<span class='badge badge-success' id='complete-".$row['id']."' data-task='".$row['id']."'>Completed</span>";

    } elseif ($row['status'] == 4) {

        echo "<span class='badge badge-info' id='approved-".$row['id']."' data-task='".$row['id']."'>Approved</span>";
    ...
    if ($row['status'] != '2') {
        ...
        // Display "Over Due" if the task is not completed and it is past the due date
            echo "<span class='badge badge-danger mx-1' id='overdue-".$row['id']."' data-task='".$row['id']."'>Over Due</span>";
        }
    }
