        ...
        <table class="table table-hover table-bordered" id="list">
            ...
            <tbody>
                ...
                $qry = $conn->query("SELECT d.id,d.employee_id,d.page_id,d.task,d.date,d.status,e.id as employeeid,e.firstname,p.id as pageid,p.pagename
                    ...
                    INNER JOIN pages p
                    ON p.id=d.page_id $where ORDER BY id ASC");
                if ($qry->num_rows > 0)
                ...
                <tr>
                    ...
                    <td>
                        <a href="javascript:void(0)" class="edit-btn" data-id="<?php echo $row['id'] ?>" style="margin-left:2rem;">
...
<script>
    ...
    // AJAX request to check if task exists
    $.ajax({
        url: 'check_task.php',
        type: 'POST',
        data: { id: id },
        success: function(response) {
            ...
            if (data.exists) {
                // Agar task exist karta hai, alert dikhaye
                alert_toast("Task already exists.",'warning');
            } else {
                // Agar task exist nahi karta, modal open kare
