        ...
        <div class="content-wrapper">
            <?php
                ...
                for ($i = 0; $i < $fileCount; $i++) {
                    ...
                    if (!move_uploaded_file($file_tmp_name, $target_file)) {
                        ...
                        if (in_array($fileType, ['mp4', 'avi', 'mov'])) {
                            // Handle video upload here (if needed)
                        } else {
                            // Handle image upload
                            // Compress the uploaded image
                            compressImage($target_file, $target_file, 75); // Adjust the quality as needed
                        }// Adjust the quality as needed
                        // Debug statement
            
                        // Insert into database (use $compressed_file instead of $target_file)
                        $result = mysqli_query($conn, "INSERT INTO tb_img(task_id, page_id, file_name, file_type, created_at, updated_at) VALUES('$id', '$page_id', '$filenamerand', '$fileType', '$created_at', '$updated_at' )");
                        if (!$result) {
                            $errors[] = "Error inserting file details into database: " . mysqli_error($conn); // Debug statement
                            $uploadSuccess = false;
                        }
                        if (empty($page_id)) {
                            // If page_id is not provided, insert only task_id.
                            $query = "INSERT INTO tb_img(task_id, file_name, file_type, created_at, updated_at) VALUES('$id', '$filenamerand', '$fileType','$created_at', '$updated_at')";
                        } else {
                            // If page_id is provided, include it in the insertion.
                            $query = "INSERT INTO tb_img(task_id, page_id, file_name, file_type, created_at, updated_at) VALUES('$id', '$page_id', '$filenamerand', '$fileType','$created_at', '$updated_at')";
                        }
                    }
                }
                ...
                if ($uploadSuccess) {
                    ...
                    if (!$result) {
                        $errors[] = "Error executing SQL query: " . mysqli_error($conn); // Debug statement
                        $uploadSuccess = false;
                    }
                }
            }
                ...
                <table cellpadding="3">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['task_id'])) {
                            ...
                            $res=mysqli_query($conn,"SELECT task_list.id,task_list.assigned_by,tb_img.*,tb_img.id as imgid
                                FROM task_list
