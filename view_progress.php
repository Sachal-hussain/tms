...
<div class="container-fluid">
	...
	<div id="post-field">
		...
		<?php 
// 		$progress = $conn->query("SELECT p.*,concat(u.firstname,' ',u.lastname) as uname 
// 			FROM task_progress p 
// 			inner join task_list t on t.id = p.task_id 
// 			inner join employee_list u on u.id = p.employee_id 
// 			where p.task_id = $id 
// 			order by unix_timestamp(p.date_created) desc ");
$progress = $conn->query("
    SELECT p.*, 
           CONCAT(COALESCE(u.firstname, admin.firstname), ' ', COALESCE(u.lastname, '')) AS uname 
    FROM task_progress p 
    INNER JOIN task_list t ON t.id = p.task_id 
    LEFT JOIN employee_list u ON u.id = p.employee_id 
    LEFT JOIN users admin ON admin.id = 2 
    WHERE p.task_id = $id 
    ORDER BY UNIX_TIMESTAMP(p.date_created) DESC
");
				...
				<div class="post">
					...
					<div class="user-block">
						...
						<?php if($_SESSION['login_type'] == 0): ?>
							...
							<span class="btn-group dropleft float-right">
								...
								<!-- <div class="dropdown-menu">
									...
									<a class="dropdown-item manage_progress" href="javascript:void(0)" data-id="<?php //echo $row['id'] ?>"  data-task="<?php //echo $row['task_id'] ?>">Edit</a>
						...
						<span class="description">
							...
							<span><b>Created Time: <?php echo $date_created; ?></b></span>
						</span>
...
<script>
	...
	$(document).on('click','.delete_progress',function(){
