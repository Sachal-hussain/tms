...
<div class="container-fluid">
    ...
    <select name="page_id[]" id="page_id" class="form-control form-control-sm" multiple required>
        <option value=""></option>
        <?php 
        // Ensure $page_id is an array for proper handling
        if (!is_array($page_id)) {
            $page_id = explode(',', $page_id); // Convert it to an array if it's a string
        }

        // Fetch pages from the database
        $pages = $conn->query("SELECT *, pagename as name FROM pages ORDER BY pagename ASC");
        while ($row = $pages->fetch_assoc()):
        ?>
            <option value="<?php echo $row['id'] ?>" 
                <?php echo isset($page_id) && in_array($row['id'], $page_id) ? 'selected' : '' ?>>
                <?php echo $row['name'] ?>
            </option>
        <?php endwhile; ?>
    </select>
